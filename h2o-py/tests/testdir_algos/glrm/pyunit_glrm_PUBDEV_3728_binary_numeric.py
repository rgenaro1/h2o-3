...
def glrm_pubdev_3728_arrest():
  ...
  try:
    glrm_h2o.train(x=prostateF.names, training_frame=prostateF, validation_frame=prostateF)
    assert False, "Should encounter error when with binary loss with categorical loss function and init=SVD for GLRM."
  except Exception as e:
    print(e)
    assert "Cannot use init mode SVD when using binary" in e.message.exception_msg, \
      "Should have raised error concerning using init mode SVD with GLRM, binary loss with binary categorical columns."
  ...
  assert pyunit_utils.equal_2D_tables(glrm_h2o._model_json["output"]["singular_vals"]._cell_values,
                                      glrm_h2o._model_json["output"]["singular_vals"]._cell_values, tolerance=1e-4), \
